...
(define-class world%
  ...
  (define/public (make s rs)
    (new world% s rs))

  (define/public (with-ship s)
    (this . make s (field rocks)))

  (define/public (with-rocks rs)
    (this . make (field ship) rs))
  ...
  (define/public (on-tick)
    (this . with-ship  (((field ship) . repel* (field rocks)) . tick)
...
(define (random-world)
  (new world% (random-ship) (repeat 10 random-rock)))
