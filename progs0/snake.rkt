...
(define seg%
  (class* cell% ()
    (super-new)
    (inherit move +img+scene)
    ;; Dir -> Seg
    (define/public (move-dir d)
      (cond [(string=? d "up")    (move 0 1)]
            [(string=? d "down")  (move 0 -1)]
            [(string=? d "left")  (move -1 0)]
            [(string=? d "right") (move 1 0)]))
    
    ;; Scene -> Scene
    (define/public (+scene scn)
      (+img+scene SEG-IMG scn))))
...
(check-expect (send origin =? origin) #t)
