...
(define-class emptyz%
  ...
  (define (to-message) empty)
  (define (kill-any-touching/a zs) zs))

(define-class conz%
  (fields first rest)
  (define (to-message) (cons (send (send this first) to-message) (send (send this rest) to-message)))
  (define (draw-on scn)
...
(define-class undead%
  ...
  (define (to-message) (list 'undead (send (send this p) to-message)))
  ;; - move-toward : Player -> Zombie
  ;; - eat-zombie-brains? : Zombie -> Boolean
  ;; - eat-player-brains? : Player -> Boolean  
...
(define-class dead%
  ...
  (define (to-message) (list 'dead (send (send this p) to-message)))
...
(define-class posn%
  ...
  (define (to-message) (list (send this x) (send this y)))
...
(define-class player%
  ...
  (define (to-message) (send (send this p) to-message))
...
(define HEIGHT 200)
(define-class za%
  (fields zombies player mouse p)
  
  ;; to-message : -> Sexp
  (define (to-message)
     (list (send (send this zombies) to-message)
           (send (send this player) to-message)
           (send (send this mouse) to-message)))
  ...
  (define (on-msg iw msg)
    (make-bundle
     (new za%
         (send this zombies)
         (send this player)
         (new mouse% (first msg) (second msg))
         (send this p))
     empty empty))
...
(define-class start%
  (define (on-msg m) (make-bundle this empty empty))
  (define (on-tick) (make-bundle this empty empty))
  (define (on-new iw)
    (send
     (new za% 
          (new conz% 
...
(define (to-z l)
  (cond [(symbol=? (first l) 'undead) (new undead% 
                                           (new posn%
                                                (first (second l))
                                                (second (second l))))]
        [(symbol=? (first l) 'dead) (new dead% 
                                         (new posn% 
                                              (first (second l))
                                              (second (second l))))]))
...
(define-class client%
  ...
  (define (on-tick) 
    (make-package this (list (send (send this mouse) x) (send (send this mouse) y))))
  ...
  (define (on-receive msg)
    (new client%
         (foldr (λ (z l) (new conz% (to-z z) l)) (new emptyz%) (first msg))
         (new player% (new posn% (first (second msg)) (second (second msg))))
         (new mouse% (first (third msg)) (second (third msg))))))

(launch-many-worlds (universe (new start%))
                    (big-bang (new client% 
                                   (new emptyz%)
                                   (new player% (new posn% 0 0))
                                   (new mouse% 0 0))))
